namespace Hospital.Server.Services
{
    ...
    public class AdminService: IAdminService
    {
       ...
       public List<Send> GetMessage(string IsRead = "ok") {
            if(IsRead!="ok")
              return Data.Send.All().Where(k => k.IsRead == false).OrderByDescending(m=>m.Id).ToList();
            else
                return Data.Send.All().OrderByDescending(m => m.Id).ToList();
        }
